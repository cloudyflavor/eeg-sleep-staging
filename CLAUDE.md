...
│   ├── features/                     # 필터·특징·정규화·문맥 (2단계)
│   ├── evaluation/                   # 분할·학습·러닝 커브 (3·4단계)
...
- 독스트링은 필요한 함수에만 **한 줄**. 이름과 타입힌트가 말하면 생략한다.
- 함정(지우면 조용히 틀리는 것)은 한 줄만 덧붙인다.
- **설계 근거·대안 비교·측정값·문헌과 레포 이름은 전부 `docs/`** — 코드에서는
  `docs/05-design-stage2.md D10` 처럼 가리키기만 한다. 양쪽에 쓰면 갈라진다.
- Parameters / Returns / Notes / References 절은 쓰지 않는다.
- 인라인 `#` 는 "왜" 만, 한 줄.
- 방식이 여러 개인 곳은 **레지스트리**(dict: 이름 → 함수·메타데이터)로 —
  `FILTERS` `NORMALIZERS` `CONTEXTS` `MODELS`. 추가는 한 줄, 모르는 이름은 예외.

## 커밋 전 검사는 훅이 강제한다

`git config core.hooksPath .githooks` 를 한 번 걸어두면 커밋할 때마다
ruff format·check·pytest 가 자동으로 돌고, **실패하면 커밋이 거부된다.**

이유: `ruff && pytest && git commit` 처럼 손으로 사슬을 엮으면 줄바꿈에서
끊긴다 (셸에서 줄바꿈은 `;` 와 같아 앞 실패와 무관하게 다음 줄이 실행된다).
실제로 이 실수를 두 번 반복해 깨진 코드가 커밋·푸시됐다.
...
### 항상 일부로 먼저, 그다음 전체 — 예외 없음

**모든 실행에 적용한다.** 대용량 처리만이 아니라 새 설정·새 조합·새 코드 전부.

```
1. --limit 2~5 로 돌린다        모양·값 범위·결측을 확인
2. 이상 없으면 전체로 확장
```

이유는 **전체를 돌리는 데 드는 시간보다 잘못 돌린 걸 알아채는 데 드는 시간이 더 비싸기
때문**이다. 24분짜리 작업을 돌려놓고 끝에 가서 설정이 틀렸다는 걸 알면 24분이 아니라
48분을 잃는다. `--limit 3` 은 30초면 끝난다.

조합을 여러 개 돌릴 때는 **조합마다 `--limit` 로 한 번씩** 먼저 확인한다.
조합 하나가 깨지면 나머지도 같이 멈춘다.
